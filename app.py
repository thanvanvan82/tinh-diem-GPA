...
# -----------------------------
EXCLUDED_CATEGORIES = {"Giáo dục quốc phòng an ninh", "Giáo dục thể chất"}
MAJORS_DATA = {
...
def get_unregistered_courses(all_sems_data, full_curriculum_df):
    if not any(not df.empty for df in all_sems_data) or full_curriculum_df.empty: return pd.DataFrame()
...
FULL_CURRICULUM_DF = pd.DataFrame(MAJORS_DATA[selected_major].get('full_curriculum', []))
...
with tab1:
    ...
    with g1:
        with st.expander("📚 Các môn chưa đăng ký học"):
            ...
            else: st.success("Chúc mừng! Bạn đã học tất cả các môn trong chương trình.")
    with g2:
        with st.expander("🎯 Các môn có thể học cải thiện (Điểm D)"):
            ...
            else: st.success("Tuyệt vời! Không có môn nào cần học cải thiện.")
